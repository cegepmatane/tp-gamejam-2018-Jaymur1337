   ...
   // public Transform StartTransform;
    ...
    private void OnDrawGizmosSelected()
    {
       ...
       // if (!DisplayPath) return;
        ...
        if (TestPath == null || TestPath.Tiles == null) return;

        for (int i = 0; i < TestPath.Tiles.Count - 1; i++)
        {
            Gizmos.DrawLine(TestPath.Tiles[i].transform.position, TestPath.Tiles[i + 1].transform.position);
        }
    }
    */
